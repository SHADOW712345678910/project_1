...
async function getSongs() {
  let a = await fetch("http://127.0.0.1:5500/arjit/");
  ...
  for (let index = 0; index < as.length; index++) {
    ...
    if (element.href.endsWith(".mp3")) {
      songs.push(element.href.split("/arjit/")[1]);
    }
  }
...
async function main() {
  ...
  number9.innerHTML = songs[8].replaceAll("%20", " ");

  let number10 = document.getElementById("number10");

  number10.innerHTML = songs[9].replaceAll("%20", " ");
